...
All in the `mcp__magicpatterns__*` namespace. Load schemas via `ToolSearch` before calling.
...
```ts
mcp__magicpatterns__create_design({
...
```ts
mcp__magicpatterns__get_design_status({ editorId })
```
...
```ts
mcp__magicpatterns__write_artifact_files({
...
```ts
mcp__magicpatterns__publish_artifact({
  artifactId: <activeArtifactId>,
  editorId
})
```
